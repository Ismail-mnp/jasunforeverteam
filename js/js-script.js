...
function initPage() {
	...
	function generateLink(lang, label) {
    let finalLabel = label;

    // Cek apakah label sudah diakhiri dengan '.html'
    if (!label.endsWith('.html')) {
        finalLabel = label + '.html';
    }

    if (lang === 'kr') {
        // Untuk bahasa Korea, langsung ke nama file
        return '/' + finalLabel;
    } else if (lang) {
        // Untuk bahasa lain yang valid, sertakan bahasa
        return '/' + lang + '/' + finalLabel;
    } else {
        // Jika 'lang' tidak ditentukan atau null/undefined, kembali ke nama file saja
        return '/' + finalLabel;
    }
}

// Mengatur atribut 'href' untuk logo Wakeone
// Perlu diingat bahwa logo biasanya mengarah ke root atau beranda,
// jadi ini tidak akan memiliki .html kecuali kamu menginginkan index.html
if (lang === 'kr') {
    $('.wakeone-logo').attr('href', '/');
} else if (lang) {
    $('.wakeone-logo').attr('href', '/' + lang + '/');
} else {
    // Jika 'lang' tidak ada, kembali ke root
    $('.wakeone-logo').attr('href', '/');
}

// Mengatur atribut 'href' untuk item menu
$('.site-menu .menu-item').each(function(){
    let label = $(this).data('menu-label'); // Ini mengambil nilai dari atribut data-menu-label
    // generateLink sekarang akan memastikan .html ditambahkan
    let link = generateLink(lang, label);
